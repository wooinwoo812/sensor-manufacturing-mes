        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-text-muted">
              {tourProgress
                ? `${tourProgress.index + 1}/${tourProgress.count}단계 · ${tourProgress.screen} · ${tourProgress.title}`
                : `${currentRole}의 실제 업무 화면을 따라가며 읽을 곳을 안내합니다.`}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {startingScreen}
            {tourProgress && onRestartTour ? (
              <Button variant="secondary" onClick={onRestartTour}>
                처음부터
              </Button>
            ) : null}
            <Button onClick={onStartTour}>
              {tourProgress ? "화면 안내 이어서 보기" : "화면 안내 시작"}
              <ArrowRight className="size-4" aria-hidden="true" />
            </Button>
          </div>
        </div>
        <p className="mt-3 text-sm leading-6 text-text-muted">
          일시중지하면 화면을 직접 사용할 수 있습니다. 상단 사용 안내나 이곳에서
          같은 단계로 돌아오세요.
        </p>
