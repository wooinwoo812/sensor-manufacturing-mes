...
} from "./material-lots-search";
it("기본 조회는 10건을 API에 명시한다", () => {
  expect(toMaterialLotsSearchParams({}).get("pageSize")).toBe("10");
});
it.each([20, 50, 100])("표시 건수 %i는 URL과 API 왕복에서 유지된다", (size) => {
  for (const raw of [size, String(size)]) {
    const search = readMaterialLotsSearch({ pageSize: raw });
    expect(search.pageSize).toBe(size);
    expect(toMaterialLotsSearchParams(search).get("pageSize")).toBe(
      String(size),
    );
  }
});
it.each([undefined, "10", "0", "-1", "11", "1000", "abc", [], {}])(
  ...
  (raw) => {
    ...
    expect(toMaterialLotsSearchParams(search).get("pageSize")).toBe("10");
  },
);
