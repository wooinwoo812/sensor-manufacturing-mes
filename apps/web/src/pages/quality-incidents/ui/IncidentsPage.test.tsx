...
describe("IncidentsPage", () => {
  it("부적합 사건 목록을 핵심 열과 함께 표시한다", async () => {
    ...
    expect(
      screen.getByText("TE 쿨링 모듈 열전달 성능 편차 의심"),
    ).toBeInTheDocument();
    expect(screen.getByText("ML-2026-0323")).toBeInTheDocument();
    expect(screen.getByText("조사 중")).toBeInTheDocument();
  ...
  it("등록 권한이 있으면 사건 등록 패널을 열 수 있다", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "사건 등록", hidden: true }),
    ).toBeInTheDocument();
    expect(screen.getByLabelText("사건 제목")).toBeInTheDocument();
  });
