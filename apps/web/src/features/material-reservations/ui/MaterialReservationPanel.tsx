...
import { Badge, Button, Panel, Input, Select, FormActions } from "@/shared/ui";
...
export function MaterialReservationPanel({
  ...
  const eligibleLots = availableLots.filter(
    (lot) =>
      requirements.length === 0 ||
      requirements.some((row) => row.materialCode === lot.materialCode),
  );
  const selectedLot =
    eligibleLots.find((lot) => lot.id === lotId) ?? eligibleLots[0];
  const required = requirements.find(
    (row) => row.materialCode === selectedLot?.materialCode,
  );
  const reserved =
    reservations
      ?.filter(
        (row) =>
          row.materialCode === selectedLot?.materialCode &&
          row.status === "ACTIVE",
      )
      .reduce((sum, row) => sum + row.quantity, 0) ?? 0;
  const maxQuantity = Math.min(
    selectedLot?.availableQuantity ?? 0,
    required ? Math.max(0, required.requiredQuantity - reserved) : Infinity,
  );
  const validQuantity =
    quantity.trim() !== "" &&
  ...
  return (
    <Panel
      ...
      description={
        isReleased
          ? `활성 예약 ${activeCount}건 · ${orderNumber}`
          : `예약부터 투입까지의 기록 · ${orderNumber}`
      }
    ...
    >
        ...
        <div className="rounded-panel border border-border p-3">
            ...
            {requirements.map((requirement) => {
              const reserved =
                reservations
                  ?.filter(
                    (row) =>
                      row.materialCode === requirement.materialCode &&
                      row.status === "ACTIVE",
                  )
                  .reduce((sum, row) => sum + row.quantity, 0) ?? 0;
              const consumed =
                reservations
                  ?.filter(
                    (row) =>
                      row.materialCode === requirement.materialCode &&
                      row.closedReason === "FULFILLED",
                  )
                  .reduce((sum, row) => sum + row.quantity, 0) ?? 0;
              return (
                <li key={requirement.materialCode}>
                  {requirement.materialName} ({requirement.materialCode}) · 필요{" "}
                  {requirement.requiredQuantity} {requirement.unit} / 예약{" "}
                  {reserved} · 투입 {consumed} {requirement.unit}
                </li>
              );
            })}
          </ul>
          {isReleased ? (
            <p className="mt-2 text-xs text-text-muted">
              필요량만큼 예약하면 첫 공정을 시작할 수 있습니다. 예약 자재는 공정
              시작 시 투입됩니다.
            </p>
          ) : null}
        </div>
      ...
      {canReserve ? (
        ...
        <p className="text-xs text-text-muted">
          예약·해제는 자재 담당자가 처리합니다. 현재 역할로는 예약 현황을 조회할
          수 있습니다.
        </p>
      )}
