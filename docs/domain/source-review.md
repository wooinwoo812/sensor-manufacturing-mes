...
| 공정 중 필수 검사 유효 `PASS` 전에는 다음 공정과 계보 변환을 차단한다. | `PROJECT` | 고신뢰성 흐름의 검사 gate 누락 반례 |
...
| `CHK-24` | 공정 중 검사 실패가 LOT 완료만 막고 다음 공정·계보는 허용 | 필수 검사 `PENDING`인 LOT을 분할한 뒤 다음 공정을 시작해 품질 gate 우회 | `ROUTE_ADVANCE` 유효 `PASS` 전 다음 공정과 모든 계보 변환 차단 |
| `CHK-25` | 검사 판정 변경을 원본 보존 정정으로 요구하지만 정정행이 없음 | 원본 PASS를 update할 수 없고 유효 FAIL도 계산할 모델이 없어 요구사항 구현 불가 | 전체 검사 snapshot의 선형 `InspectionCorrection` version과 유효 판정 계산 정의 |
