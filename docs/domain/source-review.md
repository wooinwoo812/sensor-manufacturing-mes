...
| 공정 불량수량은 MVP에서 즉시 생산 폐기하며 다음 공정 입력은 선행 공정 유효 양품수량으로 제한한다. | `PROJECT` | 공정 간 수량 보존 반례와 재작업 비범위 |
...
| `CHK-18` | 공정 내부 합계만 검증하고 공정 간 WIP를 연결하지 않음 | 1공정 투입 10·양품 8·불량 2 뒤 2공정에 10을 투입해 불량 2가 되살아남 | 공정 불량은 즉시 생산 폐기하고 후속 투입을 선행 유효 양품수량과 같게 제한 |
...
| `CHK-22` | 연결 Issue가 옛 엔터티명·완료조건·격리 처분을 유지 | 계약과 Issue를 각각 구현하면 schema와 완료 게이트가 다시 갈라짐 | #12~#16을 v1.1 계약·#22 의존관계·규칙 ID로 동시 정렬 |
