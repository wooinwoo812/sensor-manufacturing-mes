...
이 문서는 특정 기업의 내부 MES를 재현하지 않는다. 공개 제조 정보 모델을 참고하되, 단일 사업장·이산형 LOT 생산을 가정한 프로젝트 MVP의 일관된 업무 언어를 정의한다.
...
| 전이 | 허용 조건 | 금지·부수효과 |
|---|---|---|
| `DRAFT → RELEASED` | 발행된 BOM·route·검사규격, 계획수량·납기 유효, 자재·검사 요구사항 생성 가능, 하나 이상의 계획 LOT 수량 합계가 작업지시 계획수량과 일치 | revision 참조, `WorkOrderMaterialRequirement`·`InspectionRequirement`와 초기 LOT를 한 transaction에서 고정 |
...
- `COMPLETED` 공정 수량은 MVP에서 정정하지 않는다. 사용자는 완료 전에 수량을 확인해야 하며 후속 공정·계보가 생긴 뒤 잘못된 실적이 발견되면 일반 update나 WIP 증가를 허용하지 않고 별도 재작업 설계로 보낸다.
...
`DRAFT`·`IN_PROGRESS`·종결된 WorkOrder, 다른 WorkOrder의 요구사항, BOM에 없는 Material 또는 `requiredQuantity`를 초과하는 예약은 거부한다. MVP는 첫 공정 시작 전에 필요한 예약을 마쳐야 하며 진행 중 추가 예약은 지원하지 않는다.
...
| `RULE-08` | 작업지시 릴리스는 발행 revision, 자재·검사 요구사항과 유효한 LOT 계획을 원자적으로 고정한다. | BOM r3·route r2·검사 r4·합계 10 EA LOT 계획 | 릴리스 후 새 revision 발행 | 기존 지시는 고정 snapshot·계획 합계 10 유지 |
