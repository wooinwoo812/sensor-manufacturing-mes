...
- 모든 공정 시작이 자재 소비를 호출하면 첫 공정에서 이미 투입한 자재를 후속 공정이 중복 출고한다.
- 검사 판정만 있고 품질 disposition 결정 명령이 없으면 `PENDING` 생산 LOT가 완료될 수 없다.
...
- SAP 공식 Routing 문서는 BOM component를 operation에 배정할 수 있고 미배정 component를 첫 operation에 기본 배정한다고 설명한다. 이는 현재 첫 공정 투입 정책의 가능성 근거일 뿐 SAP 호환 요구가 아니다.
...
└─ 아니오
   ...
   ├─ 현재 시작 가능한지 묻는가?
   │  ├─ 예 → 첫 공정은 전체 예약과 대상 LOT 몫 확인, 후속 공정은 직전 공정·직전 검사 PASS 확인
   │  └─ 아니오 → 생산 진행 상태만 저장
   ├─ 자재가 실제 투입되는가?
   │  ├─ 첫 route 공정 → 생산 LOT 계획수량 × 단위당 소요량만 출고·소비·계보 생성
   │  └─ 후속 공정 → 자재 거래를 만들지 않음
...
5. 작업지시 릴리스는 발행된 BOM·route·검사규격, `WorkOrderMaterialRequirement`·`InspectionRequirement`와 계획수량 전체를 가진 초기 생산 LOT 하나를 한 transaction에서 고정한다. 자재 예약은 릴리스 뒤에만 가능하다.
6. 예약과 실제 소비는 같은 `WorkOrderMaterialRequirement`를 통해 WorkOrder·Material·요구수량을 검증한다. v1.0 예약은 WorkOrder가 `RELEASED`일 때만 허용하며 다른 작업지시 예약, BOM 외 자재와 요구량 초과는 거부한다.
7. 실제 투입은 유일한 초기 생산 LOT의 첫 route 공정에서만 `plannedQuantity × quantityPerProductBaseUom`만큼 출고 원장, `MaterialConsumption`, `CONSUME LotRelation`, 공정·생산 LOT·작업지시 상태와 감사를 한 transaction으로 기록한다. 후속 공정은 자재 거래를 만들지 않는다.
...
13. 생산 진행, 시작 readiness, 검사 실행·판정과 품질 disposition을 별도 상태 축 또는 projection으로 관리한다. 자재·생산 LOT disposition은 검사와 분리된 품질 담당자 명령만 바꾸며 생산 LOT `ACCEPTED`는 완료 gate와 같은 transaction에서 확정한다. readiness는 저장하지 않고 시작 명령에서 다시 검증한다.
...
| 첫 공정 자재 요구량 | domain service + decimal type + transaction + integration test | 유일한 초기 LOT이 작업지시 요구량과 다른 수량을 소비하지 않게 함 |
...
| 자재·생산 LOT 품질 결정 | API guard + aggregate lock + domain service + transaction | 권한·사유·검사 gate·격리·LOT/WorkOrder 완료를 함께 확인 |
...
- 중간 공정 투입 자재를 지원하지 않으므로 그런 제품이 범위에 들어오면 BOM item과 route step 매핑을 추가해야 한다.
- 계보 순환과 cross-row 예약 경쟁은 schema 선언만으로 해결되지 않는다.
- full event sourcing이 아니므로 임의 시점의 전체 aggregate 재생은 제공하지 않는다.

## Validation

- [제조 도메인 계약](../domain/manufacturing-domain-contract.md)의 `RULE-01`~`RULE-36`을 Given/When/Then으로 검토한다.
...
- 계획 10의 초기 LOT가 정확히 하나 생성되고 첫 공정은 요구량 10을 소비하며 후속 공정은 자재 거래를 만들지 않는지 확인한다.
- 자재 LOT의 무권한·무사유 품질 결정을 거부하고 `ACCEPTED` 전 available이 0인지 확인한다.
- 최종검사 PASS와 생산 LOT `ACCEPTED` 결정이 분리되며 승인 transaction 실패 시 LOT·WorkOrder·감사가 모두 rollback되는지 확인한다.
...
- 중간 공정에 새 자재를 투입하는 표준 제품 시나리오가 생긴다.
