# Frontend 디자인 시스템 v1 이력

과거 결정과 측정의 보존본이다. 현재 규칙은 [디자인 시스템 계약](frontend-design-system.md)을 따른다.
...
| 표 첫 열              | 가로 스크롤 시 함께 밀림                                                                              | `sticky left-0`, 오른쪽 hairline                                                                                                       | 1024px 에서 표가 옆으로 밀려도 어느 행(식별자)인지 잃지 않는다. 행 배경을 불투명으로 두어 sticky 셀이 같은 색을 받는다                                |
| 지표 타일 숫자        | 타일마다 파랑·검정·앰버·빨강                                                                          | 항상 text-strong, 예외 문구("차단 2건 포함")만 danger/warning                                                                          | 색은 예외를 표시해야지 주 수치를 물들이면 "3 이 나쁜 숫자인가"로 읽힌다                                                                               |
| 취소 사유 패널        | 화면 맨 아래 절에서 펼침                                                                              | 제목 아래(배지 행 바로 밑)에서 펼침                                                                                                    | 버튼을 눌렀는데 뷰포트 밖에서 열리면 아무 일도 없는 것처럼 보인다                                                                                     |
| 표 셀 좌우 여백       | 12px                                                                                                  | 10px, 긴 텍스트 열 최소 176px, 진행률 막대 48px                                                                                        | 1440px + 사이드바 펼침에서 chevron 열까지 가로 스크롤 없이 들어와야 한다                                                                              |
| 목록의 "상세 열기" 열 | 계보·부적합 목록에 별도 버튼 열                                                                       | 제거                                                                                                                                   | 행 전체가 입구이므로 같은 행동이 두 번 놓여 있었다                                                                                                    |
| 감사이력 코드 라벨    | 일부 행동·대상 코드가 원문 노출                                                                       | 전 코드 한국어 라벨                                                                                                                    | 서버 코드가 그대로 보이면 사용자는 자기 언어가 아니라고 느낀다                                                                                        |
