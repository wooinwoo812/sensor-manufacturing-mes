...
- 단계 수와 진행률을 제공한다. 모바일 설명 카드는 내용에 맞게 늘어나되 화면 높이의 70% 이내로 표시한다. 화면 보기를 누르면 진행 단계·설명 보기·일시중지·다음 버튼을 가진 작은 하단 바로 접고 배경의 어두운 덮개를 제거한다. 접힌 동안 업무 화면의 가로·세로 스크롤을 허용하되 입력·저장·이동 실행은 일시중지 전까지 막는다. 설명을 다시 열면 원래 대상 위치로 복귀하며 다음 단계는 설명이 펼쳐진 상태로 시작한다. PC(768px 이상)는 좌우 여백을 먼저 확인해 360~440px 범위에서 읽을 수 있는 폭으로 조절하고, 내용에 맞게 높이를 늘린다. 440px가 들어가지 않아도 360px 이상의 옆 공간이 있으면 섹션 위·아래로 밀어내지 않는다. 화면 높이를 넘는 경우에만 본문 스크롤을 사용한다. 대상 스크롤과 하단 여백은 실제 카드 크기를 기준으로 계산하여 넓은 강조 영역을 가리지 않는다. 안내 제목과 이전·다음 버튼은 스크롤 영역 밖에 유지하고 설명 본문만 스크롤한다. 단계 제목·본문을 재마운트하거나 투명도 애니메이션으로 숨기지 않는다. 설명은 본문과 같은 글꼴의 16px·500 굵기·26px 줄 간격과 본문 색으로 읽으며 스크롤바 공간을 항상 확보한다. 일시중지는 아이콘·청록 테두리·배경을 가진 명확한 버튼으로 표시한다. 넓은 섹션 아래/위 카드는 섹션 중앙에 맞춘다. 설명을 읽는 동안 카드 내부 스크롤은 허용하지만 배경 스크롤과 업무 입력·실행은 차단한다.
