...
**온보딩 지원 환경(2026-09-10): 단계별 화면 안내만 PC에서 제공한다.** 1024px 미만 화면 또는 마우스 호버가 없는 터치 중심 기기에서도 로그인·업무 메뉴·목록·상세·입력은 그대로 사용한다. 이 환경에서는 온보딩을 자동 시작하지 않으며, 사용 안내 버튼이나 업무 가이드의 투어 실행을 누르면 PC에서 안내를 실행하라는 짧은 설명만 표시한다. 세션·주소·저장된 안내 진행 상태는 유지한다.
...
모든 표 열은 기본 말줄임을 적용한다. 긴 제품명·코드·차단 사유·설명·요청 ID도 예외로 줄바꿈하지 않는다. 이름과 보조 코드처럼 의도한 두 줄은 각 줄을 따로 말줄임한다. 배지 아이콘은 유지하고 긴 라벨을 말줄임하며 마우스 오버로 셀 원문을 확인한다.
...
| 폼 행동     | `FormActions`              | 위 20px, 구분선 아래 16px, gap 12px, 오른쪽 정렬 |
...
상세 화면의 업무 액션은 본문과 이력 뒤 하단에 배치한다. 발행·취소·계보 추적은 제목 영역에 놓지 않으며, 검사 판정 입력도 변경 이력 다음에 둔다. 입력·설명과 실행 버튼을 같은 줄에 섞지 않고 `FormActions`의 구분선 아래 오른쪽에 모은다. 보조 행동 다음에 주요 확정 버튼을 둔다. 목록으로 돌아가기와 기록 간 이동 링크, 개별 예약 행의 해제처럼 해당 기록에 속한 조작은 문맥을 유지한다.
...
재조회 중 기존 표와 조회·조건 초기화 버튼의 투명도를 낮추지 않는다. 필터 버튼은 검색·초기화 아이콘과 안내 문구를 유지하고, 중복 실행만 막는다. 표 제목 옆 표시 건수는 재조회 중에도 유지한다. 요청이 400ms 이상 걸릴 때만 표 본문 안에 로딩을 표시하고 응답 시 바로 제거한다. 빠른 응답에는 로딩 표시를 생략하며 화면 이동마다 전역 로딩 알림을 띄우지 않는다. 저장 등 업무 명령 버튼의 로딩 아이콘은 원래 버튼 폭 안에 표시한다. 자동 온보딩 질문은 첫 본문 준비 뒤 열되 사용 안내 버튼 자체는 헤더에서 유지한다.

열 정렬은 `오름차순 → 내림차순 → 해제` 순서로 순환한다. 정렬 해제 시 URL의 sort/order를 제거해 API 기본 순서로 돌아가고, 필터·표시 건수는 유지한 채 첫 페이지로 이동한다. 명시적인 정렬이 없을 때 머리글에는 활성 정렬 화살표를 표시하지 않는다.

상세 하단의 `목록으로`·`작업지시로`는 secondary 색상을 유지한다. `자재 예약`·`계보 추적하기`처럼 다음 업무로 이동하는 버튼은 primary 강조색을 사용한다.
...
- 내용 확대 때문에 기존 완료·건너뛰기 선택을 초기화하지 않는다. 처음 시작은 동의를 묻고, 진행 중인 안내는 상단 사용 안내와 업무 가이드 첫 화면에서 이어서 볼 수 있다. 업무 가이드는 화면 안내 진입을 문서 목록보다 먼저 보여 준다. 페이지 이동 안내는 표 하단 탐색 영역 전체를 강조하며, 실제 페이지·표시 건수·업무 데이터는 투어가 변경하지 않는다.
- 일시중지 버튼·Escape·브라우저 뒤로가기는 배경과 입력 제한을 해제하고 진행 위치를 보존한다. 재개 알림은 숨길 수 있으며 숨겨도 상단 사용 안내와 업무 가이드에서 재개할 수 있다. 명시적인 투어 종료·안내 완료는 진행 기록을 지운다. 업무 가이드의 처음부터는 첫 단계로 돌아간다.
- 진행 위치와 선택한 상세 기록 식별자만 같은 탭의 sessionStorage에 사용자·역할별로 저장한다. 새로고침 후 자동으로 화면을 이동하지 않고 일시중지 상태를 복원한다. 저장소 차단 시 메모리로 동작한다. 같은 대상을 설명하는 여러 단계는 고유 단계 식별자로 구분하며 현재 권한에 없는 단계는 복원하지 않는다. 재개 시 실제 대상을 다시 찾고 미저장 입력·저장 중 작업을 보호한다.
- 실제 업무 배경이 보이는 반투명 강조를 유지한다. 전체 배경을 불투명하게 덮거나 가짜 업무 화면으로 대체하지 않는다.
- 스크롤은 320ms 감속을 사용한다. 스크롤 완료 후 실제 대상에 초점을 주며, 안내 카드는 정수 픽셀의 `left`·`top`으로 배치한다. 텍스트가 별도 합성 레이어에서 흐려지지 않도록 지속적인 transform·will-change와 이동 애니메이션은 쓰지 않는다. 모바일 카드는 하단의 일정한 크기·위치를 유지하며 동작 줄이기 설정을 존중한다.
- 단계 수와 진행률을 제공한다. 모바일 카드 높이는 320px로 유지한다. PC(768px 이상)는 좌우 여백을 먼저 확인해 240~440px 범위에서 읽을 수 있는 폭으로 조절하고, 내용에 맞게 높이를 늘린다. 440px가 들어가지 않아도 240px 이상의 옆 공간이 있으면 섹션 위·아래로 밀어내지 않는다. 카드 폭이 320px 미만이면 일시중지와 단계 이동 버튼을 두 줄로 배치한다. 화면 높이를 넘는 경우에만 본문 스크롤을 사용한다. 대상 스크롤과 하단 여백은 실제 카드 크기를 기준으로 계산하여 넓은 강조 영역을 가리지 않는다. 안내 제목과 이전·다음 버튼은 스크롤 영역 밖에 유지하고 설명 본문만 스크롤한다. 단계 제목·본문을 재마운트하거나 투명도 애니메이션으로 숨기지 않는다. 설명은 본문과 같은 글꼴의 16px·500 굵기·26px 줄 간격과 본문 색으로 읽으며 스크롤바 공간을 항상 확보한다. 일시중지는 아이콘·청록 테두리·배경을 가진 명확한 버튼으로 표시한다. 넓은 섹션 아래/위 카드는 섹션 중앙에 맞춘다. 카드 내부 스크롤은 허용하지만 배경 휠·터치·키보드 스크롤과 업무 입력·실행은 차단한다.
- 버튼 아래에 `← 이전 · → 다음 · Esc 일시중지`를 표시한다. 좌우 방향키는 버튼과 같은 이동을 수행하고, 첫 단계 이전 이동·화면 준비 중 이동·길게 누르기에 의한 반복 이동을 막는다. 마지막 단계의 오른쪽 키는 안내를 완료한다. 조합 입력과 Alt·Ctrl·Meta·Shift 조합키는 단계 이동으로 처리하지 않는다. 일시중지·종료하면 단축키 이벤트를 해제한다.
