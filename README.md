...
[체험 데모](https://fabriscope.pages.dev/login) · [업무 안내](docs/product/operations-guide.md) · [로컬 실행](#로컬-실행)

가상 센서 제조 공정의 작업지시·자재·생산·검사를 관리하는 개인 MES

| 항목 | 내용 |
| --- | --- |
| 담당 범위 | 작업지시·공정·검사 화면, API, 데이터 모델 구현 |
| 사용 기술 | React, TypeScript, NestJS, PostgreSQL, Prisma |
| 화면 안내 | 단계별 온보딩은 PC에서 제공 · 모바일에서도 로그인과 업무 화면 이용 가능 |
...
지원 회사의 업무와 연결되는 프로젝트를 만들고자, 센서 제조 공정을 가정한 MES를 개발했습니다. 화면 개발 경험을 바탕으로 업무에 필요한 기능과 데이터 처리까지 연결할 수 있다는 점을 보여주려 했습니다.

## 구현한 내용

### 업무 흐름

작업지시부터 자재·생산·검사까지 이어지는 화면과 API, 데이터베이스를 구현했습니다. 작업 상태와 수량 조건을 화면과 서버에서 함께 확인하도록 했습니다.

### 검사 처리

검사 결과에 따라 후속 공정의 진행 여부를 제어하고, 판정을 다시 검토해도 최초 판정과 변경 이력을 남기도록 구성했습니다.

### 변경 일관성

판정·후속 공정·감사기록을 하나의 트랜잭션으로 처리했습니다. 실제 PostgreSQL에서 중복 판정 방지와 저장 실패 시 전체 롤백을 확인했습니다.

[생산 상태 전이](apps/api/src/process-executions/production-flow.ts) · [실제 PostgreSQL 검증](apps/api/test/production-flow.postgres.spec.ts) · [검증 범위](docs/engineering/testing-and-demo.md)

## 데모 살펴보기

별도 가입 없이 로그인 화면의 **데모 둘러보기**로 최고관리자 대시보드에 접속할 수 있습니다. **담당 업무별로 접속**에서는 생산계획·현장 작업·자재·품질 담당자의 화면과 권한을 비교할 수 있습니다.

PC에서는 상단 **사용 안내**로 역할별 화면을 따라갈 수 있습니다. 안내를 일시중지해 업무 화면을 사용한 뒤 같은 단계에서 재개할 수 있으며, ←·→ 방향키로 이전·다음 단계, Esc로 일시중지할 수 있습니다. 모바일에서도 업무 화면을 사용할 수 있고, 단계별 안내만 PC에서 제공합니다.

대시보드의 **처음 살펴볼 작업 → 작업 확인**에서는 다음 사례를 확인할 수 있습니다.
...
로그인 후 메뉴의 **업무 가이드**에서도 같은 문서를 읽을 수 있습니다.
